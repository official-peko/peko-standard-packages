/*
 * peko_crypto_legacy.c
 * MD5 and SHA-1 implementations for legacy compatibility.
 * These algorithms are cryptographically broken and must not be used
 * for security. They are provided only for compatibility with existing
 * systems that need them, such as checksums and non-security protocols.
 *
 * The implementations are Brad Conte's public domain reference code,
 * wrapped with a Peko-compatible GC allocation and streaming API.
 */
